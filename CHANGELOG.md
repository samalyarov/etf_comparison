...
Turns the Portfolio page into a full portfolio-construction desk — bonds & income with
Dutch tax, factor exposures, a constrained optimiser, and a formal downside-risk engine —
all fed by a researched ETF look-through dataset, and re-skins the whole app in the new
*Meridian* design system. Additive across the board: no breaking changes to the data model,
CLI, or public module APIs.
...
- **ETF strategy / exposure profiles** (`etf/profiles.py`, pure; `scripts/etf_profiles.yaml`,
  ...
  are invented** (CLAUDE.md §4). Stored DRY as an index library + fund→index map; the seed
  survives a DB rebuild (git-ignored `data/etf_profiles.yaml` overrides if present, mirroring
  the watchlist/settings pattern).
  - **Look-through upgrade:** a follow-up research pass lifted coverage from **41 full /
    51 partial** to **82 full / 10 partial** (of 92 funds) by sourcing dated (~2026-05)
    region/country/GICS-sector/top-10 breakdowns (and credit-quality/country for bonds) from
    justETF/issuer factsheets — widening the optimiser's sector/region/asset-class constraint
    coverage across most portfolios. Every recorded weight was verified against the fund's own
    factsheet (fund identity cross-checked against the DB, since justETF-via-fetch occasionally
    serves a different fund for less-liquid share classes), with an explicit `Other` remainder
    keeping each breakdown summing to ~1.0. **10 funds left partial on purpose** (honest gaps,
    no fabrication): Franklin FTSE India, iShares MSCI USA SRI & MSCI EM SRI, iShares
    Digitalisation (source served the wrong fund), the diversified-commodity swap and euro
    high-yield bond (no basket/country breakdown exposed), and the four Vanguard LifeStrategy
    multi-asset funds (fund-of-funds look-through not cleanly available; `asset_mix` recorded).
  - **Exposure-aggregation API** (feeds the optimiser and risk constraints):
  ...
  - **Detail page — "Strategy & exposure · look-through"**: per-fund strategy, region/sector/
    ...
    indicator. The **Data page** gains a universe-level profile-coverage panel.
- **Factor-model portfolio builder** (`etf/factors.py`, pure): two complementary views plus a
  ...
  - *Regression-estimated exposures* — `factor_exposures(portfolio_returns, factor_returns)`
    ...
    per-coefficient standard errors and t-statistics. Standard convention: the market factor is
    an excess return so `RF` is subtracted from the portfolio too; monthly frequency; plain OLS
    with classical SEs (numpy only — no statsmodels/scipy). Raises `InsufficientData` on too
    little overlap.
  - *Factor-ETF building blocks* — `sleeve_contributions(prices, weights)` decomposes a
  ...
  - **Factor return data** (Ken French Data Library — European factors): new ingest adapter
    ...
    (falls back to a committed fixture if the download rate-limits). Note: the European factors
    are USD-denominated, so loadings read as relative *tilts* rather than currency-exact betas.
  - **Portfolio page — "Factor model" section**: pick MSCI World factor sleeves
    ...
    a plain-English tilt read), and (c) the best/base/worst + market-crash scenario fan.
- **Constrained portfolio optimiser** (`etf/optimizer.py`, pure numerics): mean-variance
  ...
  - **Exposure constraints use the profiles look-through** (`profiles.portfolio_exposure`
    ...
    silently constrained on incomplete data. Caps act on absolute covered exposure.
  - **Portfolio page — "Optimiser" section**: candidate set, objective, leverage / shorting,
    per-fund cap, per-sector/region caps, min-bonds floor, and an optional turnover limit
    anchored to pasted holdings. Shows optimal weights vs equal-weight, the **efficient
    frontier** with the tangency / min-vol / equal-weight / per-fund points, the resulting
    exposure breakdown + coverage, and binding constraints. Infeasible sets show a clear
    message (no crash); currency- and theme-aware; respects the risk-free-rate setting.
- **Portfolio risk engine** (`etf/risk.py`, pure — numpy/pandas only): formal downside-risk
  ...
  - **Value-at-Risk + Conditional VaR / Expected Shortfall** at 95% and 99% by **three
    independent methods** plus Monte-Carlo, reported as positive loss magnitudes with drift-aware
    **√t horizon scaling** (Basel square-root-of-time rule; documented i.i.d. assumption):
    **Historical** (empirical tail quantile, no distribution assumption); **Parametric
    (Gaussian)** with the closed-form normal ES `σ·φ(z)/(1−c) − μ`; a **Cornish-Fisher**
    fat-tail variant (modified VaR + Boudt-Peterson-Croux modified ES) that adjusts for sample
    skew and excess kurtosis and reduces **exactly** to the Gaussian forms at zero
    skew/kurtosis; and **Monte-Carlo** (seedable → deterministic) — univariate normal/bootstrap,
    or a **multivariate-normal** fit across funds preserving their covariance. Cornish-Fisher
    returns `NaN` (never a fabricated number) when the expansion leaves its domain of validity
    on the tail-to-median interval the VaR actually traverses (Maillard 2012). `var_summary(...)`
    builds the method × confidence table.
  ...
  - **Historical stress tests** (`stress_tests`) — replay of seven documented, dated crash
    ...
    **GFC 2008**, **Euro crisis 2011**, **China/oil 2015-16**, **2018 Q4 selloff**,
    **COVID 2020**, **2022 rate shock** and **SVB banking 2023**. Each reports portfolio
    drawdown, worst single day, window return and time-to-recovery; windows a portfolio has no
    data for (young funds) are reported **uncovered** with `NaN` losses — coverage is explicit,
    never fabricated.
  - **Portfolio page — "Risk" section**: the VaR/CVaR table (method × confidence) with a
...
- **Visual redesign — the *Meridian* design system** (`etf/theme.py`, `.streamlit/config.toml`,
  ...
  institutional finance direction: a deep **slate-navy** desk (Dark: plane `#0e1620`, card
  `#141f2b`, ink `#e7edf3`) and a warm **paper** desk (Light: plane `#e9e7e0`, card `#f6f5f1`,
  ink `#1b2733`), unified by a single muted **teal** accent (Light `#0c6b62` / Dark `#4bc5b5`)
  with reserved green/red up-down semantics. Both Light and Dark themes and the Native/EUR
  currency toggle keep working; all native-widget text fixes and themed HTML tables are preserved.
  ...
  - **Chrome:** a refined masthead (rising-line product mark + wordmark), a signature **meridian
    rule**, **tabular figures** on metrics/tables, and accessible keyboard focus rings.
  ...
  - **Charts:** the CVD-safe categorical series palettes are unchanged and stay in-band against
    the new surfaces; the diverging ramps keep their red/blue poles with a neutral midpoint
    tuned to each surface.

### Tests
- New suites merged across the program: `tests/test_bonds.py`, `tests/test_tax.py`,
  `tests/test_profiles.py`, `tests/test_factors.py`, `tests/test_optimizer.py`,
  `tests/test_risk.py`, and `tests/test_theme.py` (WCAG AA contrast contract), plus
  data-integrity invariants (total-return reconciliation for distributing bonds, `factor_returns`
  decimals/no-lookahead, dated & lookahead-free crash windows, no fabricated losses on uncovered
  windows) and Portfolio/Detail/Data UI smoke coverage for every new section in both themes and
  both currency modes. Suite grew **129 → 250**; ruff clean.
...
[Unreleased]: https://github.com/samalyarov/etf_comparison/compare/v1.1.0...HEAD
[1.1.0]: https://github.com/samalyarov/etf_comparison/compare/v1.0.0...v1.1.0
